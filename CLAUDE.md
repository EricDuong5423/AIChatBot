...
Không có test suite (pytest/unittest). Verify thay đổi bằng cách gọi `/chat` trực tiếp qua dashboard hoặc curl.
...
scripts/build_index.py / POST /docs/rebuild → rag.build_vectorstore()
    ...
    → Supabase pgvector (langchain_pg_embedding table, HNSW index, persist vĩnh viễn)
```

**Embedding**: Jina v3 API (`https://api.jina.ai/v1/embeddings`) — multilingual bi-encoder MTEB top tier. Lý do dùng API thay vì local model: Render free tier 512MB RAM không đủ cho HF model (~540MB). Free 1M token/tháng. **Đổi provider/dim = phải re-index** — chạy `scripts/build_index.py`. Task mode: `retrieval.passage` cho docs, `retrieval.query` cho query (asymmetric retrieval).
...
chạy trong `asyncio.to_thread` (vì PGVector sync) → trả top-8 chunks ≤800 ký tự kèm `source_url` → LLM tổng hợp.

`api.py` có `@app.on_event("startup")` warmup gọi `query_vectorstore("warmup", 1)` để verify Jina API key + Supabase pgvector connection trước khi nhận traffic. Nếu collection chưa có thì tự rebuild từ docs/.
...
Supabase đã có bảng `history` (phiên chat) và `chatbox` (từng tin nhắn). **Python tự fetch và lưu lịch sử** — C# chỉ cần truyền `history_id`:
...
// Khuyến nghị: truyền history_id (Python tự quản lý chatbox)
// C# chỉ cần: tạo history record, lưu history_id, truyền vào mỗi request
var response = await _httpClient.PostAsJsonAsync(
    $"{baseUrl}/chat?format=tmp",
    new {
        message = userMsg,
        history_id = currentHistoryId,  // UUID của phiên, từ bảng history
        user_id = currentUserId          // optional, chỉ cho logging
    }
);
...
**Luồng hoàn chỉnh:**
1. User login → C# lấy `userId` từ Supabase JWT
2. User tạo / chọn conversation → C# tạo/đọc record trong bảng `history`, lấy `historyId`
3. Mỗi tin nhắn → C# gửi `{ message, history_id, user_id }` lên Python
4. Python đọc lịch sử từ `chatbox WHERE history_id = ?` → chạy Pipecat → trả lời → lưu cả user+assistant vào `chatbox`
5. C# hiển thị response — không cần tự lưu

**Backward compat** (nếu không dùng history_id): vẫn có thể truyền `history` array trực tiếp như cũ.

```csharp
// Cách cũ — vẫn hoạt động, nhưng C# phải tự quản lý history[]
var response = await _httpClient.PostAsJsonAsync(
    $"{baseUrl}/chat?format=tmp",
